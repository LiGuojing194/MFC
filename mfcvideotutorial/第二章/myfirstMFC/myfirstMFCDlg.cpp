...
BEGIN_MESSAGE_MAP(CMyfirstMFCDlg, CDialog)      //消息映射函数
...
//OnInitDialog初始化对话框消息WM_INITDIALOG当对话框还未显示出来的时候，做准备工作
BOOL CMyfirstMFCDlg::OnInitDialog()
{
	CDialog::OnInitDialog();
    CListCtrl*pList=(CListCtrl*)GetDlgItem(IDC_LIST);      //获取控件指针或句柄
	pList->InsertColumn(0,"工号：",LVCFMT_LEFT,120);
	pList->InsertColumn(1,"姓名：",LVCFMT_LEFT,120);
	pList->InsertColumn(2,"工资：",LVCFMT_LEFT,120);
